...
export function DesignTokens() {
    return (
        <div className="space-y-20">
            <header>
                <h2 className="text-xs font-bold text-deep-umber/40 uppercase tracking-[0.4em] mb-4">Section 03</h2>
            ...
            {/* Colors */}
            <section>
                <h3 className="text-xs font-bold text-charcoal uppercase tracking-widest mb-10 border-b border-deep-umber/10 pb-4">Color Palette</h3>
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-8">
                    {COLORS.map((color) => (
                        <div key={color.name} className="flex gap-4 items-center">
                            <div className={`w-20 h-20 shrink-0 shadow-sm ${color.class}`} />
                            <div>
                                <h4 className="text-[10px] font-bold text-deep-umber/40 uppercase tracking-widest mb-1">{color.type}</h4>
            ...
            <section className="space-y-12">
                <header className="border-b border-deep-umber/10 pb-4">
                    <h3 className="text-xs font-bold text-charcoal uppercase tracking-widest mb-2">Typography Scale</h3>
                ...
                <div className="grid lg:grid-cols-2 gap-16">
                    ...
                    <div className="space-y-8">
                        <h4 className="text-[10px] font-bold text-deep-umber/40 uppercase tracking-widest border-l-2 border-ochre pl-3">Headings</h4>
                            ...
                            {GEIST_HEADINGS.map(h => (
                                <div key={h.id} className="group">
                                    <div className="flex justify-between items-end mb-1">
                                        ...
                                        <span className="text-[9px] font-mono text-deep-umber/40 uppercase">{h.size} / {h.line}</span>
                                    </div>
                                    <h5
                                        className={`text-charcoal font-bold uppercase truncate text-heading-${h.id}`}
                                    >
                                        Nairobi Art
                                    </h5>
                                </div>
                    ...
                    <div className="space-y-12">
                        <div className="space-y-8">
                            <h4 className="text-[10px] font-bold text-deep-umber/40 uppercase tracking-widest border-l-2 border-rich-blue pl-3">Copy (Body)</h4>
                            <div className="space-y-8">
                                {GEIST_COPY.map(c => (
                                    <div key={c.id}>
                                        <div className="flex justify-between items-end mb-2">
                                            <span className="text-[9px] font-mono text-deep-umber/40 lowercase">text-copy-{c.id}</span>
                                            <span className="text-[9px] font-mono text-deep-umber/40 uppercase">{c.size} / {c.line}</span>
                                        </div>
                                        <p
                        ...
                        <div className="pt-8 border-t border-deep-umber/5">
                            <h4 className="text-[10px] font-bold text-deep-umber/40 uppercase tracking-widest border-l-2 border-charcoal pl-3 mb-6">Font Families</h4>
                            <div className="grid grid-cols-2 gap-8">
                                <div className="space-y-2">
                                    <span className="text-[9px] font-mono text-deep-umber/40 uppercase">Sans (Primary)</span>
                                    <p className="text-xl font-bold text-charcoal">Geist Sans</p>
                                </div>
                                <div className="space-y-2">
                                    <span className="text-[9px] font-mono text-deep-umber/40 uppercase">Mono (System)</span>
