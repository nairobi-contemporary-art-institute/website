...
export function EducationFilter({ programs, locale }: EducationFilterProps) {
    ...
    const tEvents = useTranslations('Pages.events')
    const [filter, setFilter] = useState('all')

    const AUDIENCE_FILTERS = [
        { label: t('filterAll') || 'All', value: 'all' },
        { label: 'UJUZI', value: 'ujuzi' },
        { label: t('filterSchools') || 'Schools', value: 'youth' },
        { label: t('filterFamilies') || 'Families', value: 'children' },
        { label: t('filterPublic') || 'Public', value: 'adults' },
    ]

    const filtered = filter === 'all'
        ? programs
        : programs.filter(p => p.audience === filter)
        ...
        <div className="space-y-12">
            ...
            <section>
                <div className="flex items-center justify-between mb-8">
                    <h2 className="text-3xl font-light tracking-tight">{tEvents('upcoming')}</h2>
                ...
                {upcoming.length > 0 ? (
                    ...
                    <div className="py-32 text-center border border-dashed border-charcoal/20 bg-stone-50/50">
                        <p className="text-charcoal/40 font-mono capitalize tracking-widest text-sm">{t('noPrograms')}</p>
                    </div>
                )}
            </section>
            ...
            {past.length > 0 && (
                <section className="pt-20 border-t border-rich-blue/20">
                    <div className="flex items-center gap-4 mb-8">
                        <h2 className="text-2xl font-light text-charcoal/60">{tEvents('past')}</h2>
